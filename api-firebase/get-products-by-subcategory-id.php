...
if (isset($_POST['accesskey']) && isset($_POST['subcategory_id'])) {
    ...
    if ($access_key_received == $access_key) {
        ...
        $sql = "SELECT *,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p WHERE subcategory_id='" . $subcategory_id . "' AND `status`=1 $sort LIMIT $offset, $limit";
        $db->sql($sql);
        $res = $db->getResult();
        $product = array();

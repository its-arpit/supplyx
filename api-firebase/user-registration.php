...
if ((isset($_POST['type'])) && ($_POST['type'] == 'register')) {
	...
	if (!empty($mobile)) {
		...
		if ($num_rows > 0) {
			...
			$sql = "INSERT INTO `users`(`name`, `email`,`profile`, `mobile`,`dob`, `city`,`area`, `street` , `pincode`, `apikey`, `password`,`referral_code`,`friends_code`,`fcm_id`,`latitude`,`longitude`,`status`) VALUES 
			('$name','$email','$filename','$mobile','$dob','$city','$area','$street','$pincode','$api_key','$password','$referral_code','$friends_code','$fcm_id','$latitude','$longitude',$status)";
			$data = array(
				'name' => $name,
				'email' => $email,
			...
			$db->sql($sql);
			$res = $db->getResult();
			$usr_id = $fn-> get_data($columns = ['id'],'email = "' . $email . '"', 'users');

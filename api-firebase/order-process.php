...
if (isset($_POST['place_order']) && isset($_POST['user_id']) && !empty($_POST['product_variant_id'])) {
    ...
    $sql = "INSERT INTO `orders`(`user_id`,`otp`,  `mobile`, `total`, `delivery_charge`, `tax_amount`, `tax_percentage`, `wallet_balance`, `discount`, `promo_code`, `promo_discount`, `final_total`, `payment_method`, `address`, `latitude`, `longitude`, `delivery_time`, `status`, `active_status`) VALUES ('$user_id','$otp_number','$mobile','$total','$delivery_charge','$tax_amount','$tax_percentage','$walletvalue','$discount','$promo_code','$promo_discount','$final_total','$payment_method','$address','$latitude','$longitude','$delivery_time','$order_status','$active_status')";
    $db->sql($sql);
    ...
    if ($db->update('orders', $data, 'id=' . $order_id)) {
        ...
        if ($active_status == "received") {
            ...
            // send_email($support_email, $subject, $message); //remaining

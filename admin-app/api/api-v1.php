...
if (isset($_POST['get_customers']) && !empty($_POST['get_customers'])) {
   ...
   */
    ...
    if (isset($_POST['city']) && !empty($_POST['city'])) {
        $filter_user = $db->escapeString($fn->xss_clean($_POST['city']));
...
if (isset($_POST['get_products']) && !empty($_POST['get_products'])) {
   ...
   */
    ...
    if (isset($_POST['search']) && !empty($_POST['search'])) {
        $search = $db->escapeString($fn->xss_clean($_POST['search']));
        $where .= " and `id` like '%" . $search . "%' OR `name` like '%" . $search . "%' OR `slug` like '%" . $search . "%' OR `category_id` like '%" . $search . "%' OR `subcategory_id` like '%" . $search . "%' OR `manufacturer` like '%" . $search . "%' OR `made_in` like '%" . $search . "%' OR `return_status` like '%" . $search . "%' OR `description` like '%" . $search . "%'";
    }

    $sql = "SELECT count(id) as total FROM products WHERE `status`=1 $where";
    $db->sql($sql);
    $res = $db->getResult();
    foreach ($res as $row) {
        $total = $row['total'];
    }
    $sql = "SELECT p.*,(SELECT " . $price . " FROM product_variant pv WHERE pv.product_id=p.id) as price FROM products p WHERE `status`=1 $where $sort LIMIT $offset, $limit";
    $db->sql($sql);
    $res = $db->getResult();
    $product = array();
...
if (isset($_POST['get_financial_statistics']) && !empty($_POST['get_financial_statistics'])) {
   ...
   */
    ...
    if (!empty($total_orders) && !empty($total_products) && !empty($total_users)) {
